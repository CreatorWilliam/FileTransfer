...
class SendViewController: UIViewController {
  ...
  fileprivate var reciverPeerID: MCPeerID?
...
// MARK: - Utility
fileprivate extension SendViewController {
  ...
  func scanNearbyPeer() {
    ...
    //发布广播
    self.advertiser = MCNearbyServiceAdvertiser(peer: self.peerID, discoveryInfo: nil, serviceType: "rsp-sender")
    self.advertiser?.delegate = self
    self.advertiser?.startAdvertisingPeer()
    
    //收听广播
    self.browser = MCNearbyServiceBrowser(peer: self.peerID, serviceType: "rsp-receiver")
  ...
  func setupView() {
    ...
    self.navigationItem.title = "发送文件"
...
extension SendViewController: MCSessionDelegate {
  ...
  func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
    ...
    case .connected:
      ...
      print("已连接")
      guard let name = self.fileItem?.title else { return }
      guard let url = self.fileItem?.url else { return }
      self.reciverPeerID = peerID
      
      self.session?.sendResource(at: url, withName: name, toPeer: peerID, withCompletionHandler: { (error) in
        
        guard let error = error else { return }
        
        print("发送源数据发生错误：\(error.localizedDescription)")
      })
